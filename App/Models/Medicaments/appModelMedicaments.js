...
Medicament.updateMedicament = (id, medicament, result) => {
    sql.query("UPDATE medicament SET ? WHERE MED_DEPOTLEGAL = ?;", [medicament, id], (err, res) => {
        if(err) {
            result(null, err);
        } else {
            result(null, res);
...
Medicament.deleteMedicament = (id, result) => {
    sql.query("DELETE FROM medicament WHERE MED_DEPOTLEGAL = ?;", [id], (err, res) => {
        if(err) {
            result(null, err);
        } else {
            result(null, res);
...
Medicament.getAllMedicaments = (numPage, result) => {
    ...
    sql.query("SELECT * FROM medicament LIMIT 10 OFFSET ? ;", offset, (err, res) => {
        if(err) {
            result(null, err);
        } else {
            result(null, res);
...
Medicament.getMedicamentById = (medicamentId, result) => {
    sql.query("SELECT * FROM medicament WHERE MED_DEPOTLEGAL = ? ", medicamentId, (err, res) => {
        if(err) {
            result(err, null);
        } else {
            result(null, res);
